...
public class MainActivity extends AppCompatActivity {
        ...
        send.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (bitStringLength == 0)
                {
                    ...
                    chirpConnect.send(padMessage(correctText + crc_g(bitStringLength, correctText) + currSequenceNo));
                }
                    ...
                    chirpConnect.send(padMessage(toBeSent + crc_g(bitStringLength, correctText) + currSequenceNo));
                ...
                if (messageSent){
                    ...
                    else if (recdBitString.equals(NACK + currSequenceNo))
                    {
                        runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                Toast.makeText(getApplicationContext(),
                                        "Received NACK, Retrying!" ,
                                        Toast.LENGTH_SHORT).show();
                            }
                        });
                        isTimeoutRunning = false;
                        timeoutThread.quit();
                        chirpConnect.send(padMessage(correctText + crc_g(correctText.length(), correctText) + currSequenceNo));
                    }
    ...
    private byte[] padMessage(String message) {
        ...
        char[] chars = new char[padLength - 1];
        Arrays.fill(chars, '0');
        return getByteByString((new String(chars)) + "1" + message);
    ...
    public static byte[] getByteByString(String binaryString) {
        ...
        if(binaryString.length() % splitSize == 0){
           ...
           Log.v("Error","Cannot convert binary string to byte[], because of the input length. '"
                   +binaryString+"' % 8 != 0");
            return null;
        }
    }
