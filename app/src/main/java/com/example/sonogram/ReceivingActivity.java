    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ConnectEventListener connectEventListener = new ConnectEventListener() {
            ...
            public void onReceived(byte[] payload, byte channel) {
                ...
                String temp = stripPadding(payload);

                final int seqNo = Integer.parseInt(temp.substring(temp.length() - 1));
                bitString = temp.substring(0, temp.length() - 1);
                if (CRCToolkit.error_d(bitString.length(), bitString)) {
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            SpannableStringBuilder builder = new SpannableStringBuilder();
                            SpannableString str1 = (SpannableString) messageBoxes[seqNo].getText();
                            ...
                            builder.append(str1);
                            builder.append(str2);
                            messageBoxes[seqNo].setText(builder, TextView.BufferType.SPANNABLE);
                            ...
                            chirpConnect.send((ACK + seqNo).getBytes());
//                            messageNo = (messageNo + 1) % 2;
    ...
    private static String stripPadding(final byte[] b) {
        ...
        return String.valueOf(bits).replaceFirst("^0*1", "");
    }
}
